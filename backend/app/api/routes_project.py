...
async def create_project(payload: dict, db: AsyncSession = Depends(get_db)):
    ...
    return await service.create_project(payload.get("title", "Untitled"), payload.get("description", ""))


@router.get("")
async def list_projects(page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: AsyncSession = Depends(get_db)):
    service = ProjectService(db)
    return await service.list_projects(page, size)
...
async def update_project(project_id: uuid.UUID, payload: dict, db: AsyncSession = Depends(get_db)):
    service = ProjectService(db)
    ok = await service.update_project(project_id, **payload)
    if not ok:
        raise HTTPException(status_code=404, detail="Project not found")
    return {"ok": True}
...
async def get_versions(project_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    service = ProjectService(db)
...
async def save_version(project_id: uuid.UUID, payload: dict, db: AsyncSession = Depends(get_db)):
    service = ProjectService(db)
...
async def get_latest_version(project_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    service = ProjectService(db)
    versions = await service.repo.get_versions(project_id)
    if not versions:
...
async def get_version(project_id: uuid.UUID, version: int, db: AsyncSession = Depends(get_db)):
    service = ProjectService(db)
    versions = await service.repo.get_versions(project_id)
