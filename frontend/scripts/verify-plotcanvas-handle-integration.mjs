...
assert.match(source, /isHandlePairAvailable\(conn\.source, conn\.target, conn\.sourceHandle, conn\.targetHandle, edges, ignoreEdgeIds, edgeType\)/)
assert.match(source, /isHandlePairAvailable\(newConn\.source, newConn\.target, newConn\.sourceHandle, newConn\.targetHandle, edges, ignoreEdgeIds, edgeType\)/)
assert.match(source, /const edgeType = connectionMode === 'all' \? 'timeline' : connectionMode/)
assert.match(source, /该侧连接点已被占用/)
assert.match(source, /conn\.source, conn\.target, edgeType, conn\.sourceHandle, conn\.targetHandle/)
